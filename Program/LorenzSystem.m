...
%% Newton shooting method to find PO
% u = (x, y, z)
% f^T(u) - u = 0 

max_iter = 200; % maximum number of Newton iterations
% solve
X0 = [x0, tmax];
func = @(X) Newton_shooting(X, x0, dt, sigma, beta, rho);
options = optimoptions('fsolve', 'Display', 'iter', 'MaxIterations', max_iter);
U = fsolve(func, X0, options);

return 

%% Variational dynamics to find PO
